...
const Header = () => {
  ...
  // ✅ Auto-scrolling slider (no DOM mutation)
  useEffect(() => {
  ...
  return (
    <>
      {/* ✅ Top Auto Scrolling Header */}
      <div className="header-wrapper hidden sm:flex overflow-hidden bg-[#202D42] text-white h-12 items-center px-4 text-sm">
<div className="top-slider flex gap-8 whitespace-nowrap items-center overflow-x-scroll no-scrollbar">

          {Array(2).fill(0).map((_, idx) => (
            <React.Fragment key={idx}>
              {/* Slide 1 */}
              <div className="flex items-center gap-2 px-4">
                <p>ExpertMD introduces faster, more accurate medical billing for practices of all sizes.</p>
                <Link href={'/services/medical-billing'} className="underline">Learn more</Link>
              </div>

              {/* Slide 2 */}
              <div className="flex items-center px-4">
                <p>We now offer full RCM automation including eligibility checks, claim scrubbing, and denial management.</p>
              </div>

              {/* Slide 3 */}
              <div className="flex items-center gap-2 px-4">
                <p>ExpertMD’s 2024 Revenue Optimization Report is now available.</p>
                <Link href={'/resources/reports/revenue-optimization'} className="underline">Click here to view.</Link>
              </div>
            </React.Fragment>
          ))}
